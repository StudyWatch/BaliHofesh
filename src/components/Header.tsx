...
const Header: React.FC = () => {
  ...
  // --- תפריטי ניווט עם תרגום ---
  const mainMenu: Array<{ href: string; label: string; icon: any }> = [
    { href: '/', label: t('nav.home'), icon: Home },
    { href: '/tutors', label: t('nav.tutors'), icon: GraduationCap },
    { href: '/tips', label: t('nav.tips'), icon: Lightbulb },
    { href: '/store', label: t('nav.store'), icon: ShoppingCart },
  ];
          ...
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center min-w-0 select-none group">
              ...
              <div className="hidden sm:flex flex-col mr-4 min-w-0">
                <span className="text-xs text-gray-500 truncate">{t('home.title')}</span>
      ...
      <div className="container mx-auto px-2 sm:px-4">
        <div className="flex items-center justify-between h-16">
          ...
          <Link to="/" className="flex items-center min-w-0 select-none group" tabIndex={0}>
            ...
            <div className="hidden sm:flex flex-col mr-4 min-w-0">
              <span className="text-xs text-gray-500 truncate">{t('home.hero.title')}</span>
              <Badge variant="secondary" className="text-xs mt-1">Beta</Badge>
            </div>
          </Link>
          ...
          <div className="flex items-center space-x-1 xs:space-x-2 sm:space-x-4 space-x-reverse">
            ...
            <button
              ...
              `}
              aria-label={isDarkMode ? t('common.save') : t('common.loading')}
              title={isDarkMode ? t('common.save') : t('common.loading')}
              type="button"
              style={{
            ...
            {user && user.email_confirmed_at && (
              ...
              >
                ...
                {unreadCount > 0 && (
                  <span className="absolute -top-1.5 -right-1.5 min-w-[1.4rem] h-5 px-1 bg-red-500 text-white text-xs font-bold flex items-center justify-center rounded-full shadow border-2 border-white animate-bounce" style={{ zIndex: 2 }} aria-label={`${unreadCount} ${t('notifications.title')}`}>
